...
class UnauthorizedUserApiTests(TestCase):
    ...
    def test_create_same_user_denied(self):
        payload = {
            "email": "test@test.com",
            "password": "test12345",
        }
    ...
    def test_create_token_with_not_existing_user(self):
        payload = {
            "email": "test@test.com",
            "password": "test12345",
        }
    ...
    def test_create_token_with_wrong_data(self):
        res = self.client.post(TOKEN_URL, {"email": 1, "password": ""})
        self.assertNotIn("token", res.data)
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)

    def test_retrieve_user_denied(self):
...
class AuthorizedUserApiTests(TestCase):
    ...
    def test_retrieve_post_denied(self):
